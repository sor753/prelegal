...
export default function MndaPage() {
  ...
  async function handleSave() {
    ...
    const title = company ? `MNDA - ${company}` : `MNDA - ${new Date().toLocaleDateString("ja-JP")}`;
    try {
      const res = await fetch("/api/documents", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ doc_type: "mnda", title, form_data: formData }),
      });
      ...
      <header className="sticky top-0 z-10 bg-white border-b shadow-sm">
        <div className="max-w-screen-xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-500">相互秘密保持契約 (MNDA)</span>
